...
def get_scatter_plot(entered_site, payload):
    ...
    data = data[min_payload <= data['Payload Mass (kg)']]
    data = data[max_payload >= data['Payload Mass (kg)']]
    if entered_site == 'ALL':
        fig = px.scatter(data, x='Payload Mass (kg)', y='class', color="Booster Version Category")
        return fig
    else:
        data = data[data['Launch Site']==str(entered_site)]
        fig = px.scatter(data, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig
